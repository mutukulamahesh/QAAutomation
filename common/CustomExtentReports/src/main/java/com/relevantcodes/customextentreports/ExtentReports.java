...
import com.relevantcodes.customextentreports.converters.ReportParser;
import com.relevantcodes.customextentreports.converters.TestConverter;
...
public class ExtentReports extends Report implements Serializable {
	...
	public ExtentReports(String filePath, Boolean replaceExisting, DisplayOrder displayOrder, NetworkMode networkMode, Locale locale) {
		...
		DuplicateReportCode.startSuite=suiteTimeInfo.getSuiteStartTimestamp();
		DuplicateReportCode.lastOperation=DuplicateReportCode.startSuite;
		DuplicateReportCode.mainMethod=DuplicateReportCode.mainMethod+String.valueOf(DuplicateReportCode.startSuite)+";\n";
		DuplicateReportCode.mainClass=DuplicateReportCode.mainClass+"public static long rTime="+String.valueOf(DuplicateReportCode.startSuite)+"L;\n";
	...
	public synchronized ExtentTest startTest(String testName, String description) {
		ExtentTest test = new ExtentTest(testName.replaceAll("\r","").replaceAll("\n", ""), description.replaceAll("\r","").replaceAll("\n", ""));
		updateTestQueue(test);
		return test;
	}


	public synchronized ExtentTest startTest(long time,String testName, String description) {
		System.out.println("completed "+testName);
		ExtentTest test = new ExtentTest(time,testName, description);
		updateTestQueue(test);
		return test;
	}
	...
	public synchronized void endTest(long time,ExtentTest extentTest) {
		if (extentTest != null && !extentTest.getInternalTest().isChildNode) {
			Test test = extentTest.getInternalTest();
			...
			if (!test.hasEnded) {
				...
				DuplicateReportCode.endTest=time;
				finalizeTest(time,test);
			}
		}
	}
	...
	public synchronized void close(long time) {
		terminate(time);
