...
import com.relevantcodes.customextentreports.converters.TimeConverter;
import com.relevantcodes.customextentreports.model.ExceptionInfo;
import com.relevantcodes.customextentreports.model.Log;
import com.relevantcodes.customextentreports.model.SuiteTimeInfo;
...
abstract class Report extends LogSettings implements Serializable {
	...
	/**
	 * <p>
	 * Default configuration file for HTML report. This file is loaded by
	 * default when the {@link ExtentReports} initializes, and overridden when
	 * user provides their own configuration using <code>loadConfig(args)</code>
	 * .
	 */
	...
	protected String getRunDuration() {
		...
		if (DuplicateReportCode.endTest == -1) {
			...
			cal.setTimeInMillis(DuplicateReportCode.endTest);
			currentSuiteRunDuration = DateTimeUtil.getDiff(cal.getTime(),
					new Date(suiteTimeInfo.getSuiteStartTimestamp()));
		}
	...
	protected String getRunDurationOverall() {
		/*
		 * if (totalDurationPastRun == 0) { if (currentSuiteRunDuration == null)
		 * { getRunDuration(); }
		 *
		 * return currentSuiteRunDuration; } long millis=0;
		 * if(DuplicateReportCode.endTest==-1) { millis =
		 * (Calendar.getInstance().getTime().getTime() - startedTime.getTime());
		 * } else{ millis = (DuplicateReportCode.endTest -
		 * startedTime.getTime()); } millis += totalDurationPastRun;
		 *
		 * long hours = TimeUnit.MILLISECONDS.toHours(millis); long mins =
		 * TimeUnit.MILLISECONDS.toMinutes(millis); long secs =
		 * TimeUnit.MILLISECONDS.toSeconds(millis); millis -= secs * 1000;
		 *
	...
	protected Map<String, List<Test>> getCategoryTestMap() {
		...
		Map<String, List<Test>> a = new LinkedHashMap<String, List<Test>>();

		if (categoryTestMap.containsKey("sanity")) {
			a.put("sanity", categoryTestMap.get("sanity"));
		}
		if (categoryTestMap.containsKey("minimumRegression")) {
			a.put("minimumRegression", categoryTestMap.get("minimumRegression"));
		}
		if (categoryTestMap.containsKey("fullRegression")) {
			a.put("minimumRegression", categoryTestMap.get("fullRegression"));
		}
		if (categoryTestMap.containsKey("targetedRegression")) {
			a.put("minimumRegression", categoryTestMap.get("targetedRegression"));
		}

		for (Map.Entry<String, List<Test>> entry : categoryTestMap.entrySet()) {
			if (!entry.getKey().equals("minimumRegression") && !entry.getKey().equals("sanity")) {
	...
	protected void finalizeTest(Test test) {
		...
		endTime = new Date(Calendar.getInstance().getTime().getTime());

		DuplicateReportCode.currMethod = DuplicateReportCode.currMethod + "WSLib.report.endTest(rTimeInc("
				+ String.valueOf(Calendar.getInstance().getTime().getTime() - DuplicateReportCode.lastOperation)
				+ "),WSLib.logger);\nWSLib.report.flush();\n}\n";
		DuplicateReportCode.lastOperation = Calendar.getInstance().getTime().getTime();
		DuplicateReportCode.methods.append(DuplicateReportCode.currMethod);
		...
		// all
		// recorded as the time at which the report is generated. This causes
		// the
		// log timestamps to be greater than the test ended time. Below fix
		// ensures
		// the log timestamp is always equal to or between test start and end
		// times
	...
	protected void finalizeTest(long time, Test test) {

		endTime = new Date(DuplicateReportCode.endTest);

		DuplicateReportCode.currMethod = DuplicateReportCode.currMethod + "fun3(rTimeInc("
				+ String.valueOf(Calendar.getInstance().getTime().getTime() - DuplicateReportCode.lastOperation)
				+ "));\n}\n";
		DuplicateReportCode.lastOperation = Calendar.getInstance().getTime().getTime();
		DuplicateReportCode.methods.append(DuplicateReportCode.currMethod);
	...
	private void updateTestStatusList(Test test) {
		...
		toAdd = test.getStatus() == LogStatus.FATAL || test.getStatus() == LogStatus.ERROR
				|| test.getStatus() == LogStatus.WARNING || test.getStatus() == LogStatus.UNKNOWN ? true : false;
	...
	protected void terminate() {
		...
		DuplicateReportCode.mainMethod = DuplicateReportCode.mainMethod + "fun4(rTimeInc("
				+ DuplicateReportCode.endSuite + "));\n}";
	...
	protected void terminate(long time) {
		for (ExtentTest extentTest : testList) {
			Test test = extentTest.getInternalTest();
	...
	public void flush() {
		...
		DuplicateReportCode.endSuite = new Date().getTime() - DuplicateReportCode.lastOperation;

		// #320 - ensure atleast 1 test is added, otherwise skip the flush
		// process
		if (getTestList() != null) {
	...
	public void flush(long time) {
		...
		suiteTimeInfo.setSuiteEndTimestamp(time);
	...
	protected Report() {
		/*
		 * String resourceFile = Report.class .getPackage() .getName()
		 * .replace(".", "/") + "/resources/" + CONFIG_FILE;
		 */
		System.out.println("Extent Report Config File from resources " + CONFIG_FILE);
