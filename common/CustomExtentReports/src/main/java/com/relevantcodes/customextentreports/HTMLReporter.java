...
import com.relevantcodes.customextentreports.utils.Resources;
import com.relevantcodes.customextentreports.utils.Writer;
import com.relevantcodes.customextentreports.view.Icon;
...
public class HTMLReporter extends LogSettings implements IReporter {
	...
	public void start(Report report) {
		...
		ResourceBundle resourceBundle = ResourceBundle.getBundle("com.relevantcodes.customextentreports.view.localized", getDocumentLocale());
	...
	private void initOfflineMode(File file) {
		...
		String cssPath = resourcePackagePath;
		String fontsPath = cssPath;
		String jsPath = resourcePackagePath;
		String imagesPath = resourcePackagePath;
		...
		String[] folderNames = { "images","css" + s + "fonts", "js" };
		...
		for (String f : images) {
			Resources.moveResource(imagesPath + f, destPath + "images" + s + f);
		}
		for (String f : fonts) {
	...
	public synchronized void flush() {
		...
		try {
			...
			try {
				...
				extentSource=extentSource.replace("<!-- xml Modal Replacements -->", StaticValues.xmlModal);

				extentSource=extentSource.replace("<!-- raw xml Replacements -->", StaticValues.rawXmlModal);




				extentSource=extentSource.replace("<!-- raw ErrorLogs Replacements -->", StaticValues.rawErrorData);


				extentSource=extentSource.replace("<!-- ErrorLogs Replacements -->", StaticValues.errorData);

				HashMap<String, String> opertime = OperationTime.generateOperationTime();

				extentSource=extentSource.replace("<!-- time table Replacements -->",opertime.get("html"));

				extentSource=extentSource.replace("<!-- time Modal Replacements -->",opertime.get("timeModal"));
		...
		Writer.getInstance().write(new File(parent+"\\TimeMachine.java"), DuplicateReportCode.mainClass+"\n"+DuplicateReportCode.methods+"\n}");
	}
