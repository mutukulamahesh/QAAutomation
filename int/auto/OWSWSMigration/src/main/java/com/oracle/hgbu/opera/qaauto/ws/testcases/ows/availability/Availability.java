...
public class Availability extends WSSetUp
{
	...
	//Sanity
	@Test(groups = { "sanity","OWS","Availability","Availability", "availability_4980" })
	...
	public void availability_4980(String runOnEntry)
	{
		try
		{
			...
			WSClient.startTest(testName, "Verify that rooms are available for the given rate code and the roomtype in case of General availability", "sanity");
	...
	public void availability_22929() {
		...
		try {
			...
			WSClient.startTest(testName, "Verify that user is able to see error message if reservation is created for more rooms than maximum Number of Rooms Sold per Reservation. ", "minimumRegression");
	...
	public void availability_22430() {
		...
		try {
			...
			WSClient.startTest(testName, "Verify that the Requested Rate Code details are fetched when the channel parameter REQUESTED_BAR_PUBLIC_RATES is set to 'REQUESTED'", "minimumRegression");
			String uname = OPERALib.getUserName();
			OPERALib.setOperaHeader(uname);
			String interfaceName = OWSLib.getChannel();
			String resortOperaValue = OPERALib.getResort();
