...
public class ReportFormatter {
	...
	public static String formatMessage(LogStatus status, String message) {
		if (status == LogStatus.PASS) {
			...
			return "<font color=\"blue\">" + message + "</blue>";
		}
		return "<font color=\"black\">" + message + "</font>";
	}


	public static String printReqResp(String request, String response, String operation, String statusCode,String operationKeyword) {
			...
			if(logSplit[i].length()!=0){
				if(logSplit[i].charAt(0)=='['){
					if(i>0){
						...
						first=1;
						subLog=logSplit[i];
						subLog="<b>"+subLog.substring(0,subLog.indexOf("]")+1)+" : </b>"+subLog.substring(subLog.indexOf("]")+1);
